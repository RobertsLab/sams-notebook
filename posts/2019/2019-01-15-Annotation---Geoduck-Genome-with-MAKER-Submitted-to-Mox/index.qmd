...
I've submitted the job to Mox and now we wait. When I last [ran this process on the Olympia oyster genome](https://robertslab.github.io/sams-notebook/posts/2018-11-27-Annotation---Olurida_v081-MAKER-on-Mox/), the job took about two weeks to complete. I've made some changes that will allow it to run a bit faster (I think/hope) - primarily by using gene model GFFs for the SNAP gene model trainings, instead of relying on the FastA file file each time, since the FastA file needs to get BLASTed on each round and BLASTing is a time consuming process./
