...
```{bash head-C_virginica-3.0_Gnomon_exon-geneID.bed}
...
#| eval: FALSE
gunzip --keep /home/shared/8TB_HDD_01/sam/gitrepos/ceabigr/genome-features/GCF_002022765.2_C_virginica-3.0_genomic.gff.gz
```
...
So, let's look at the data Steven was working with and see what we can figure out.

# Inspect `output/19-exon-expression/S12M-exon_expression.tab`

## Colums in `output/19-exon-expression/S12M-exon_expression.tab`
...
# Compare `e_data.ctab` and `GCF_002022765.2_C_virginica-3.0_genomic.gff`
...
exon.count.ncbi.gff <- system("awk -F \"\t\" '$3 == \"exon\"' /home/shared/8TB_HDD_01/sam/gitrepos/ceabigr/genome-features/GCF_002022765.2_C_virginica-3.0_genomic.gff | wc -l")
```
