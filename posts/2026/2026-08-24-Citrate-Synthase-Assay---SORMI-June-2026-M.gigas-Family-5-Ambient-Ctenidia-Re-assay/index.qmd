...
toc-depth: 5
toc-location: left
title: Citrate Synthase Assay - SORMI June 2026 M.gigas Family 5 Ambient Ctenidia Re-assay
date: '2026-08-24'
draft: false
engine: knitr
categories: 
...
This is a re-assay of the eight *Magallana gigas* (Pacific oyster) Family 5 ctenidia homogenates held at **ambient** temperature, using the Citrate Synthase Assay Kit (ABCAM; cat: ab239712). The [original assay of these same homogenates on 20260811](../2026-08-11-Citrate-Synthase-Assay---SORMI-June-2026-M.gigas-Family-5-Ambient-Ctenidia/index.qmd) (Notebook entry) had QC failures serious enough that its activity values could not be used — a failed GSH standard curve, standards above the photometric linear range, background control wells behaving as active reactions, and one sample (`F05_05_ambient`) with no clean replicate. This plate is the re-run called for by that entry.

Two things changed relative to the 20260811 run: a **shorter read window** (20 min / 11 reads, vs. 40 min / 21 reads) and a **pooled background control design** in place of paired per-sample backgrounds. Samples had been [previously homogenized on 20260811](../2026-08-11-Homogenization---SORMI-June-2026-M.gigas-Ctenidia-from-Families-5-and-7-for-Citrate-Synthase-Assay/index.qmd) (Notebook entry), and protein concentrations for normalization were [determined on 20260813](../2026-08-13-Protein-Quantification---June-2026-SORMI-M.gigas-Ctenidia-from-Famillies-5-and-7-for-Citrate-Synthase-Assay/index.qmd) (Notebook entry) — the same homogenates and the same protein measurements used for the original assay.

::: {.callout-note}

**This re-assay succeeded and supersedes the 20260811 ambient results.** No sample well, positive-control well, or background well is flagged for anything on this plate: no decreasing kinetics, no read glitches, no elevated starting absorbance, and all three pooled-background replicates are flat. All eight samples pass the 15% technical CV threshold (0.8-4.6%), the GSH standard curve has zero flagged outlier wells (R^2^ = 0.9983), and no replicate is excluded from any sample. **This plate does not need re-assay.**

One caution worth carrying forward: the positive control's three replicates rise linearly (R^2^ 0.997-1.000) but disagree in rate by **22.8% CV** (`D7` at 2.00 mA412/min vs. `D8`/`D9` at 2.95-3.15 mA412/min) — worse precision than any individual sample's technical triplicate on this plate. This does not enter any activity calculation, so it is a plate-level caution rather than a disqualifier, but it is worth watching on future plates.
...
The Citrate Synthase Assay Kit (ABCAM; cat: ab239712) was used according to the manufacturer's instructions ([ABCAM-Citrate-Synthase-Assay-v4a-ab239712.pdf](https://github.com/RobertsLab/resources/blob/master/protocols/Commercial_Protocols/ABCAM-Citrate-Synthase-Assay-v4a-ab239712.pdf) (GitHub; PDF)). The kit is a coupled kinetic assay: citrate synthase condenses acetyl-CoA and oxaloacetate to release free CoA-SH, the liberated thiol reduces DTNB to TNB^2-^, and the rate of absorbance increase at 412 nm is proportional to citrate synthase activity. Absolute nmol of thiol are assigned from a reduced glutathione (GSH) standard curve read on the same plate.
...
Activity was normalized to total extracted protein rather than tissue weight. Tissue was homogenized in 350 µL of Assay Buffer 7, and protein concentration in each homogenate was [quantified on 20260813](../2026-08-13-Protein-Quantification---June-2026-SORMI-M.gigas-Ctenidia-from-Famillies-5-and-7-for-Citrate-Synthase-Assay/index.qmd) (Notebook entry) using the Bradford assay. Total protein mass per sample is that concentration multiplied by the 350 µL homogenization volume. Tissue weights are recorded in the plate layout labels for provenance only and play no role in the activity calculation. Protein values come from two assays run on 20260813: the F05 plate (`F05_04`, `F05_05`, `F05_07`) and the F05/F07 re-assay plate (`F05_01`, `F05_02`, `F05_03`, `F05_06`, `F05_08`, re-assayed at dilution).

## Analysis

Data were analyzed in R. The Rmd file is here (GitHub):

- [Gen5-20260824-mgig-sormi-citrate_synthase-F05-ambient.Rmd](https://github.com/RobertsLab/sormi-assay-development/blob/main/Citrate_synthase/code/Gen5-20260824-mgig-sormi-citrate_synthase-F05-ambient.Rmd)
